...
| **3 — Content / assessment** | Syllabus weighting verified vs PRC + applied; recalibration run once on real data; mastery heatmap on a real taxonomy; AI review loop processed one batch | **In progress** — decomposed into 3.1–3.5. **3.1 theta-engine unification DONE** (3PL is the single estimator for thetaRating+standardError; recompute backfill). Next: 3.2 taxonomy+syllabus (needs PRC weights), 3.3 recalibration, 3.4 BKT+heatmap, 3.5 AI review loop (confidence-scale decision) |
