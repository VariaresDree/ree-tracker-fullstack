...
// Highly robust Regex to extract the exact 11-character YouTube Video ID 
// from any variation (youtu.be, watch?v=, mobile links, etc.)
...
export default function MediaViewer({ item }) {
  ...
  switch (item.type) {
    case 'video':
      ...
      // If it's a YouTube link, extract the ID and force the authorized embed format
    ...
    case 'audio':
      return (
        <div className="w-full p-6 bg-surface border border-border2 rounded-xl shadow-sm flex flex-col items-center gap-4 mt-8">
          <div className="w-20 h-20 rounded-full bg-reePurple/10 border border-reePurple/30 flex items-center justify-center text-4xl shadow-[0_0_15px_rgba(139,92,246,0.2)] animate-pulse">
            🎙️
          </div>
    ...
    case 'pdf':
      return (
        <div className="w-full h-[80vh] rounded-xl overflow-hidden border border-border2 shadow-sm bg-surface2">
          <iframe src={`${item.url}#toolbar=0`} className="w-full h-full" title={item.title}></iframe>
        </div>
      );
