...
export default function LibraryOverview({ serverStats, vaultMetadata, resyncVaultMetadata, manualMode, setManualMode }) {
  ...
  const setDynamicTOS = useStore((state) => state.setDynamicTOS);
  const isAdmin = useStore((state) => state.stats?.role === 'admin');
  ...
  const handleResync = async () => {
    ...
    try {
      ...
      toast.success("Matrix tally successfully resynchronized with global vault.");
